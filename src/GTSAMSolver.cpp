...
void GTSAMSolver::AddConstraint(karto::Edge <karto::LocalizedRangeScan> *pEdge) {
    ...
    graph_.emplace_shared < BetweenFactor < Pose2 > >
    (sourceID, targetID, Pose2(diff.GetX(), diff.GetY(), diff.GetHeading()), model);
...
void GTSAMSolver::WriteGraphFile(std::string name) {
    // save result to File
